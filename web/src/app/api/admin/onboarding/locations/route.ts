...
export async function GET(req: NextRequest) {
  ...
  const rows = await db.select().from(onboardingLocations)
    .where(eq(onboardingLocations.organizationId, organizationId))
...
export async function POST(req: NextRequest) {
  ...
  const [item] = await db.insert(onboardingLocations).values({
